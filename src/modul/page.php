...
<? //---------------------- DETALNO
if($d==135||$d==136||$d==137||$d==138) return;
...
if($f[page_type]==1){
	...
	//$max_size_file = '500K';//get_cfg_var('upload_max_filesize');
...
<? //-------------------- SPISOK
//echo $d;
$rr=mysql_query("SELECT * FROM katalog where id_p='$d' and id_p<>'0' and menu_vizible='0' and menu_dozvil='0' and page_type<>'4' order by menu_sort ".($d==3?" desc ":" asc ") );
$horizont=3;
		...
		echo '<table border="0" width="100%" style="border:1px dotted #bbbbbb;" bgcolor="F5F9FA">
				<tr>
					<td valign=top style="padding:5px;text-align:justify">
						...
						<a href="k',$ff[id],'-',$ff[page_translit],'.html" title="',$ff[page_name],'">подробнее ЫЫ</a>
					</td>
				</tr>
			</table>';}
}
