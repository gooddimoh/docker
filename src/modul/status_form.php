						...
						<table border=0 cellpadding="0" cellspacing="0">
							<tr><td><img alt=""  src="images/zadachi/zayava.png" border=0 hspace=2></td>
								<td><a href="#" class=infobold>';
									...
									}else if($_GET[st]==5){echo 'Загрузить решение'.((int)$_GET['id']>0?' ID='.(int)$_GET['id']:'');
							...
							<tr>
								...
								<table border="0" width="100%" id="table1">
									<tr>
										...
										<td align="right"><font size="2" face="Arial" color="#28A8BC">время</font></td>
								...
								Но не позже</font></span><font size="2" face="Arial" color="#333333">: <font size="2" face="Arial" color="#28A8BC">',date('d.m.y H:i',$zadacha[end_time]),'</font></font></td>
							</tr>
							<tr>
...
echo'
							...
							echo '<form action="modul/status_form.php?id=',$_GET[id],'&st=7" method="post"  enctype="multipart/form-data">
								<table border="0" width="100%">
									...
									<tr>
										...
										<td> <img alt=""  src="images/bullet.png"></td>
										<td><input type="text" name="price" disabled size="20" style="width:350px" name="cost" class="cost" value="',$zadacha[price],'"><span lang="ru"> </span></td>
										<td>&nbsp;</td>
	...
	if($_SESSION[logintrue]){
						...
						if($end_time>=$zadacha[end_time]){
							...
							if($_POST[price]>=$zadacha[price] and $_POST[price]>=$price[price]){
                                ...
                                if($q_tmp = mysql_query("select id from katalog where page_type='6' and id_p='$_GET[id]' and id in (select id_p from katalog_zayava where userresh='$_SESSION[userid]')"))
                                    while($res = mysql_fetch_row($q_tmp))
                                    {
    									$oldzayavaid=$res[0];
								...
								mysql_query("insert into katalog_message(user_in,user_out,text,data_create,new_resh) values('$_SESSION[userid]','$zadacha[userzakaz]','Задача ID=$_GET[id]. Поступила заявка на решение','$data','".(int)$_GET[id]."')");
			...
			//download resh
				if($_GET[st]==5){
	               	if($zadacha[status]==3){
//echo $_FILES["filename"]["type"]; echo '<br/>'.print_r($_FILES) ;exit;
						if($_FILES["filename"]["size"]){
							if($_FILES["filename"]["size"]/1024<500){
								if(($_FILES["filename"]["type"]=='application/octet-stream' and (substr($_FILES["filename"]["name"],-4)=='.zip' or substr($_FILES["filename"]["name"],-4)=='.ZIP' or substr($_FILES["filename"]["name"],-4)=='.rar' or substr($_FILES["filename"]["name"],-4)=='.RAR')) or $_FILES["filename"]["type"]=='image/jpeg' or $_FILES["filename"]["type"]=='image/bmp' or $_FILES["filename"]["type"]=='image/png' or $_FILES["filename"]["type"]=='image/gif' or $_FILES["filename"]["type"]=='text/plain' or $_FILES["filename"]["type"]=='application/msword' or $_FILES["filename"]["type"]=='application/msword' or $_FILES["filename"]["type"]=='application/pdf' or $_FILES["filename"]["type"]=='application/x-rar-compressed' or $_FILES["filename"]["type"]=='application/zip'){
									...
									mysql_query("insert into katalog_message(user_in,user_out,text,data_create, resh_ok) values('$_SESSION[userid]','$zadacha[userzakaz]','Решающий задачи ID=$_GET[id] загрузил решение','$data',".(int)$_GET[id].")");
									/*
									 * 
									 * GIM Block
				...
				if($_GET[st]==6){
					if($zadacha[status]<3){
						...
						if($user[balance]>=$price){
                            ...
                            mysql_query("insert into katalog_message(user_in,user_out,text,data_create,select_resh) values('$_SESSION[userid]','$_POST[userresh]','$_SESSION[userid] $_POST[userresh] Вас выбрали решающим задачи ID=$_GET[id]','$data',".(int)$_GET[id].")");
							/*
							 * 
							 * GIM Block
							 * відправка листа - тому кого назначили виконавцем
			...
			//edit zadacha
				...
				//	if($_POST[price]!=''){
						...
						$end_time=mktime($_POST[D4],0,0,$_POST[D2],$_POST[D1],$_POST[D3]);
						if($end_time>=$zadacha[end_time]){
							mysql_query("update katalog_zadach set end_time ='$end_time',coment='$_POST[coment]',url='$url' WHERE id_p='$_GET[id]' limit 1");
