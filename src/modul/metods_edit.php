...
if(!$_SESSION[logintrue]||((int)$_GET['mid']<1&&(int)$_GET['zid']<1))  { echo '<script>document.location.href="/k136.html"</script>'; exit; }
	...
	if($_SESSION[logintrue]){ ?>
		...
		<script lang="javascript">
			function sel_vuz(){ 
				...
				jQuery.post("/modul/metods_edit.php?ajax",{country:country, city:city, vuz_name:vuz_name, kafedra:kafedra }, function(data){ jQuery("#vuzselect").html(data);})
			}
		</script>
		
		<?if((int)$_GET['mid']) { 
			$query = mysql_query('SELECT * FROM katalog_metods WHERE id = '.(int)$_GET['mid'].' AND user_id = '.(int)$_SESSION['userid'].'');
			if(!mysql_num_rows($query)) { echo '<script>document.location.href="/k136.html"</script>'; exit; }
...
<!---->		<form enctype="multipart/form-data" action="/k137.html?mid=<?=(int)$_GET['mid']?>" method=post id="form2">
...
<?if(!$res['diplom']) { ?>
						<tr>
							<td><b><font face="Arial" size="2" color="#006666">Описание задач методички:</font></b></td>
...
<? } ?>
						...
						<tr>
							<td><b><font face="Arial" size="2" color="#006666">Обложка:</font></b></td>
							<td><input type="file" name="imgname" size="20"></td>
							<td></td>
						</tr>
<?if($res['diplom']) { ?>
						<tr>
							<td><b><font face="Arial" size="2" color="#006666">Часть методички:</font></b></td>
...
<? } ?>
				...
				echo 'Методичка принята к рассмотрению, для изменения данных обратитесь к администрации'; }
		 } else { 
			$query = mysql_query('SELECT kmz.* FROM katalog_metods_zad as kmz LEFT JOIN katalog_metods as km ON(km.id = kmz.metod_id) WHERE kmz.id = '.(int)$_GET['zid'].' AND km.user_id = '.(int)$_SESSION['userid'].'');
//echo 'SELECT kzm.* FROM katalog_metods_zad as kmz LEFT JOIN katalog_metods as km ON(km.id = kmz.metod_id) WHERE kmz.id = '.(int)$_GET['zid'].' AND km.user_id = '.(int)$_SESSION['userid'].''; exit;
			if(!mysql_num_rows($query)) { echo '<script>document.location.href="/k136.html"</script>'; exit; }
			$res = mysql_fetch_array($query); 
		?>
		<form enctype="multipart/form-data" action="/k137.html?zid=<?=(int)$_GET['zid']?>" method=post id="form3">
			<table border="0" width="100%" id="table1">
				<tr>
					<td>
					</td>
				...
				<tr>
					...
					<table border="0" id="table3">
						...
						<tr>
							<td>Методичка</td>
							<? $query1 = mysql_query('select * from katalog_metods where user_id="'.$_SESSION['userid'].'" AND diplom = 0');
...
*/
		...
		if(isset($_POST['B1'])){
				...
				mysql_query('UPDATE katalog_metods SET '.($fileshorturl!=''?' fileshort_name = "'.$fileshorturl.'", ':'').''.($url!=''?' file_name = "'.$url.'", ':'').''.($imgurl!=''?' img_name = "'.$imgurl.'", ':'').' country = "'.$country.'", city = "'.$city.'", vuz_name = "'.$vuz_name.'", vuz_short_name = "'.$vuz_short_name.'", kafedra = "'.$kafedra.'", metod_name = "'.$metod_name.'", year = "'.$year.'", authors = "'.$authors.'", pages = "'.$pages.'", price = "'.$price.'", description = "'.$description.'" WHERE id = '.(int)$_GET['mid'].' AND user_id = '.(int)$_SESSION['userid'].'')or die(mysql_error());
		}
		if(isset($_POST['B2'])){
