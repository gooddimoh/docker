...
$roz = '';
if(isset($_POST[filtr])&&(int)$_POST[filtr]){ $roz = " and katalog_user.rozdil like '%".$_POST[filtr]."%' and  katalog_user.rozdil!='' ";$_SESSION[temp_form]=$_GET;}
...
if($_GET[r]==""){$_GET[r]='0';}
if($_GET[r]==2){
	//echo "SELECT katalog.*,katalog_user.rozdil,balance,dat_enter,dat_reg,email FROM katalog,katalog_user where katalog.id_p='2' and katalog.id=katalog_user.id_p ".$roz.$sqlstr.$methodstr;	
    $r = mysql_query("SELECT katalog.*,katalog_user.rozdil,balance,dat_enter,dat_reg,email FROM katalog,katalog_user where katalog.id_p='2' and katalog.id=katalog_user.id_p ".$roz.$sqlstr.$methodstr)or die(mysql_error());//підрозділи теперішнього розділу
}else{	
    $r = mysql_query("SELECT * FROM katalog where katalog.id_p='$_GET[r]' ".$sqlstr.$methodstr)or die(mysql_error());//підрозділи теперішнього розділу
}
