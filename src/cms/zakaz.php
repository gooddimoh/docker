...
if(!isset($_POST[B1])){
	if($_SESSION[logintrue]){		echo '
		<p align="center"><font size="2" face="Arial" color="#333333">Пожалуйста, заполните форму, приведенную ниже (поля, отмеченные <img src="images/bullet.png"> ,		обязательны для заполнения).</font></p>
		...
		document.getElementById(\'sub\').disabled = true;
			...
			<tr>
				<td>&nbsp;</td>
				<td>&nbsp;</td>
				<td><input type="submit" value="Добавить заказ" name="B1" disabled id="sub"></td>
				<td>&nbsp;</td>
				<td>&nbsp;</td>
			</tr>
		</table><form>';
	}else{	echo '<p align="center"><font size="2" face="Arial"><font color="#FF0000">Вы не авторизированый пользователь. Для оформления заказа
	...
	if($_SESSION[logintrue]){
		...
		if($_FILES["filename"]["size"]/1024>10000){			array_push($_SESSION[error],'Файл превышает 10 Мб, слишком большой');		}
		...
		if(count($_SESSION[error])==0){array_push($_SESSION[result],'Заказ принят');header("Location:../k6.html");}
